...
export default function HouseSheet({ house, visible, onClose, onSaved, onDeleted }: Props) {
  const { household, user } = useAuthStore();
  const { addHouse, updateHouse, deleteHouse } = useHousesStore();
  ...
  useEffect(() => {
    ...
    if (house) {
      setName(house.name);
      setColor(house.color ?? NIDO_COLORS[0].key);
    } else {
      setName('');
  ...
  const handleSave = async () => {
    if (!household) return;
    if (!name.trim()) { setError('Ponle un nombre a la casa'); return; }
    setSaving(true);
    setError('');
    const input = { name: name.trim(), color };
    try {
      if (house) {
        const res = await updateHouse(house.id, input);
        if (!res.ok) { setError(res.error ?? 'No se pudo guardar'); return; }
        onSaved?.({ ...house, ...input });
      } else {
        const res = await addHouse(household.id, user?.id, input);
        if (!res.ok || !res.house) { setError(res.error ?? 'No se pudo guardar'); return; }
        onSaved?.(res.house);
      }
      onClose();
    } finally {
  ...
  const handleDelete = () => {
    const msg = 'Los servicios de esta casa quedarán sin casa asignada. ¿Eliminarla?';
    if (Platform.OS === 'web') {
      if (typeof window === 'undefined' || window.confirm(msg)) doDelete();
      return;
    }
    Alert.alert('Eliminar casa', msg, [
    ...
    <BottomSheet visible={visible} onClose={onClose}>
      <View style={s.body}>
        <View style={s.headerRow}>
          <Text style={s.sheetTitle}>{isEdit ? 'Editar casa' : 'Nueva casa'}</Text>
          {isEdit && (
        ...
        <TextInput
          ...
          placeholder="Nombre (ej. Casa de la playa, Piso Madrid…)"
        ...
        <PressScale
          ...
          disabled={saving}
        >
          {saving ? <ActivityIndicator color={C.white} /> : <Text style={s.saveText}>{isEdit ? 'Guardar cambios' : 'Añadir casa'}</Text>}
        </PressScale>
      </View>
    </BottomSheet>
  );
